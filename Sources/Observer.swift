...
public final class Observer<E: AXElement> {
    ...
    @MainActor public fileprivate(set) lazy var application: AXApplication =
        .init(forKnownProcessID: self.pid)!
    ...
    /// - throws: `Error.NotificationUnsupported`: The element does not support notifications (note
    ///           that the system-wide element does not support notifications).
    @MainActor public func removeNotification<Element: AXElement>(
        _ notification: AXNotification,
        forElement element: Element
    ) throws {
...
private func internalCallback(
    ...
    let observer = Unmanaged<Observer<GenericAXElement>>.fromOpaque(userData).takeUnretainedValue()
...
private func internalInfoCallback(_: AXObserver,
                                  axElement: AXUIElement,
                                  notification: CFString,
                                  cfInfo: CFDictionary,
                                  userData: UnsafeMutableRawPointer?)
